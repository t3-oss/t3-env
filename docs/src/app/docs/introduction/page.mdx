...
By default, some frameworks (e.g. Next.js) treeshake away unused environment variables unless you explicitely access them on the `process.env` object. This means that the above implementation would fail even if you export and use the `envVarialbes` object in your application, as no environment will be included in the bundle for some environments / runtimes.
