...
Using Next.js or Nuxt? We have dedicated packages with preconfigured settings for these frameworks:
...
<div className="flex gap-2 mt-2">
  <Link href="/docs/nextjs" className={cn(buttonVariants({ variant: "outline" }))}>
    Next.js →
  </Link>
  <Link href="/docs/nuxt" className={cn(buttonVariants({ variant: "outline" }))}>
    Nuxt →
  </Link>
</div>
...
`worktreeDetection` is an experimental local-development workaround for Node and Bun runtimes. It is meant to help linked git worktrees share the main checkout's project-root `.env`, not to establish a new secret-storage convention.

</Callout>

When enabled, T3 Env will try to detect whether the current checkout is a linked worktree. If it is, T3 Env will treat the main worktree's `.env` file as canonical, load values from it before validation, and create a symlink in the linked worktree when that is safe to do.
...
- The canonical shared file stays at the main worktree root as `.env`.
- Existing `.env` files inside linked worktrees are left alone and loaded as-is.
- If the main worktree `.env` is missing, T3 Env warns and skips the linking behavior.
- Unsupported runtimes warn and fall back to the provided runtime environment.
