...
export const env = createEnv({
  ...
  /**
   * Remove the `client` and `clientPrefix` properties if you only want the
   * environment variables to exist on the server.
   */
  client: {
  ...
  /**
   * Treat empty strings as undefined, which is useful when validating
   * and assigning default values to environment variables that come from a
   * ".env" file. (This is set to `false` by default.)
   */
