...
In Next.js, the `runtimeEnv` is optional and will default to `process.env` if not provided. If you're using Next.js < 13.4.4,
we advise you to specify the `runtimeEnv` manually, else some might be tree shaken away by Next.js's static analysis.

When specifying it manually, `runtimeEnv` is strict by default, meaning you'll have to destructure all the keys manually.
This is due to how Next.js used to bundle environment variables and only explicitly accessed variables are included in the bundle. Missing keys will result in a type-error:
