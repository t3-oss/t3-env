...
Currently only works with the core module
...
export const serverOnlyEnv = createEnv({
  // ...
  // Tell the library when we're in a server only context,
  // which disables requirement of client fields
  serverOnly: true,
});
```
...
export const serverOnlyEnv = createEnv({
  // ...
  // Tell the library when we're in a client only context,
  // which disables requirement of server field
  clientOnly: true,
});
```
